...
namespace Repository.Pattern.Ef6
{
    public class DataContext : DbContext, IDataContextAsync
    {
        ...
        public DataContext(string nameOrConnectionString) : base(nameOrConnectionString)
        {
            ...
            //TODO разобратьс€ с этим параметром
        ...
        private void SyncObjectsStatePreCommit()
        {
            foreach (var dbEntityEntry in ChangeTracker.Entries())
            {
                ...
                //≈сли сущность имеет аудит-пол€, то заполним им их тут
                if (dbEntityEntry.State != EntityState.Added && dbEntityEntry.State != EntityState.Modified) continue;
                var type = dbEntityEntry.Entity.GetType();
                if (!typeof (IAuditableEntity).IsAssignableFrom(type)) continue;
                var entity = dbEntityEntry.Entity as IAuditableEntity;
                ...
                //var identityName = Thread.CurrentPrincipal.Identity.Name;// System.Security.Principal.WindowsIdentity.GetCurrent().Name; 
                var now = DateTime.UtcNow;
                if (dbEntityEntry.State == EntityState.Added)
                {
                    entity.UserCreate = identityName;
                    entity.DateCreate = now;
                }
                entity.UserUpdate = identityName;
