...
aboutApp.controller("AboutEditController",function($scope,$http,$location,$routeParams){
    ...
    $scope.SaveChanges = function(){
        if ($scope.AboutPartForm.$valid){
            ...
            $http.post('AboutUs/UpdateServices', fd, {
    ...
    $scope.AddPhoto = function(){
        ...
        $http.post('AboutUs/AddPhotoToServices', fd, {
                transformRequest: angular.identity,
                headers: { 'Content-Type': undefined }
            }).success(function (response) {
                if (response == 'ok') {
    ...
    $scope.Delete = function(){
        $http.get('AboutUs/RemoveServices?id=' + $scope.Service.idRecord).success(function(response){
            $location.path('/');
        });
    }

    $scope.DeletePhoto = function(index){
        $http.get('AboutUs/RemovePhotoFromServices?id=' + $scope.Service.photos[index].idRecord).success(function(response){
            if(response == 'ok'){
                $http.get('AboutUs/GetServicesFull?id=' + $routeParams.id).success(function(response){
                        $scope.Service = new Service(response);
                });
